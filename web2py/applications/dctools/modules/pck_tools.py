...
def pck_to_model(pck):
    ...
    # get physics file
    try:
        pck_file_physics = next(iter(pck_file for pck_file in pck.get_files() if pck_file.get_ext_string() == "json"))
        new_path = os.path.join(os.path.dirname(pck_file_physics.path), model_json["physics"])
        os.rename(pck_file_physics.path, new_path)
        pck.get_file(hash=pck_file_physics.hash).path = new_path
        pck.get_file(hash=pck_file_physics.hash).ext = 1
    except Exception as e:
        pass
